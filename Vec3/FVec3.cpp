...
std::ostream& operator<<(std::ostream& p_stream, const FVec3& p_target)
{
...
std::istream& operator>>(std::istream& p_stream, FVec3& p_target)
{
...
FVec3 operator*(float p_scalar, const FVec3& p_target)
{
	return FVec3
	(
		p_target.x * p_scalar,
...
FVec3 FVec3::Slerp(const FVec3& p_start, const FVec3& p_end, float p_alpha)
{
	...
	return (sinf((1.0f - p_alpha) * theta) / sinTheta) * p_start + (sinf(p_alpha * theta) / sinTheta) * p_end;
}