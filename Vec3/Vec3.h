...
namespace lm
{
	...
	template<typename T> std::ostream& operator<<(std::ostream& os, const Vec3<T>& vec3)
	{
		os << "Vec3(" << vec3.X() << ", " << vec3.Y() << ", " << vec3.Z() << ")";
		return os;
	}

	template<class T> std::istream& operator>>(std::istream& is, Vec3<T>& vec3)
	{
		T x, y , z;
 		is >> x;
		is >> y;
	...
	template<class T> std::string operator+(const std::string& str, const Vec3<T>& vec3)
	{
		return str + "(" + std::to_string(vec3.X()) + ", " + std::to_string(
			vec3.Y()) + ", " + std::to_string(vec3.Z()) + ")";
	}

	template<class T> void operator+=(std::string& str, const Vec3<T>& vec3)
	{
		str += "(" + std::to_string(vec3.X()) + ", " + std::to_string(vec3.Y()) + ", " +
